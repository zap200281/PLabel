  ...
  for (var i=0;i<dataSetTaskData.length;i++){
    if (i==sindex){
        var row = "<option value=\""+dataSetTaskData[i].id+
              ...
              "</option>";
        $("#datasetlabeltaskname").attr({value:dataSetTaskData[i].task_name + "-视频标注"});
    }else{
        var row = "<option value=\""+dataSetTaskData[i].id+
...
function sele_Change(sele){
  ...
  $("#labeltaskname").attr({value:predictTaskName+"-视频标注"});
}
...
function dataset_sele_Change(sele){
  ...
  $("#datasetlabeltaskname").attr({value:dataset_listName+"-视频标注"});
}
  ...
  for (var i=0;i<preDictTaskData.length;i++){
    if (i==sindex){
        var row = "<option value=\""+preDictTaskData[i].id+
              ...
              "</option>";
        $("#labeltaskname").attr({value:preDictTaskData[i].task_name + "-视频标注"});
    }else{
        var row = "<option value=\""+preDictTaskData[i].id+
...
function list(current,pageSize){
      $.ajax({
       type:"GET",
       url:ip + "/api//video-label-task-page",
       headers: {
          authorization:token,
        },
       dataType:"json",
...
function display_list(){
 ...
 for (var i=0;i<tableData.length;i++){
    if(isEmpty(tableData[i].videoInfo)){
      continue;
    }
    var row = "<tr>\
            ...
            <td>" + 
			"<a onclick=\"personLabel(\'" + tableData[i].id + "\' "+"," + "\'" + tableData[i].zip_object_name + "\' "+"," + "\'" + tableData[i].task_name + "\' "+"," + "\'" + tableData[i].videoInfo.duration + "\' )\" class=\"btn btn-xs btn-success\">开始"  + "视频标注</a>" + "&nbsp;&nbsp;&nbsp;<a onclick=\"setTaskId(\'"+tableData[i].id+"\');\" class=\"btn btn-xs btn-success\">导出标注数据</a>" + "&nbsp;&nbsp;&nbsp;<a onclick=\"modify_taskstatus(\'"+tableData[i].id+"\');\" class=\"btn btn-xs btn-success\">修改</a>" 
...
function submit_modify_taskstatus(){
	...
	$.ajax({
		 type:"PATCH",
		 url:ip + "/api/video-label-task-status",
		 dataType:"json",
		 async:false,
		 headers: {
			authorization:token, 
...
function personLabel(task_id, zip_object_name,task_name, task_video_duration){
    ...
    sessionStorage.setItem('label_task_video_duration',task_video_duration);

	window.location.href="labelingVideoBox.html";
}
...
    var taskreturnid = "";
	$.ajax({
	   type:"GET",
	   url:ip + "/api/video-label-download/",
	   headers: {
		  authorization:token,
		},
	   dataType:"json",
