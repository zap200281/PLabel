...
var dataset_id = sessionStorage.getItem("dataset");
var datasetInfo;
...
if(!isEmpty(datasetInfo)){
	if(datasetInfo.datasetType == 4){
		if(datasetInfo.total == 0){//视频预览
			files  = ip+ datasetInfo.zip_object_name;//"/minio/label-img/1586502176987-test111.mp4";
  ...
  else if (datasetInfo.datasetType == 3){
      files  = ip+ datasetInfo.zip_object_name;//"/minio/label-img/1586502176987-test111.mp4";
            console.log("files:",files);
      var filename = files.substring(files.lastIndexOf("/") + 1);
      $("#bigImgfilename").text(filename);
        document.getElementById("myVideo_div").style.display="none";
        document.getElementById("myCanvas_div").style.display="none";
        document.getElementById("myBigImg_div").style.display="block";
        showBigImg();
  }
  else{//图片预览
	    document.getElementById("myVideo_div").style.display="none";
...
function showBigImg(){
    var width = document.getElementById("myBigImg_div").offsetWidth;
